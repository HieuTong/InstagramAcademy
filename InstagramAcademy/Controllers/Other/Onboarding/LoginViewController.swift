...
class LoginViewController: UIViewController {
    ...
    private func configureHeaderView() {
        ...
        imageView.frame = CGRect(x: headerView.width / 4.0, y: view.safeAreaInsets.top
                                 , width: headerView.width / 2.0, height: headerView.height - view.safeAreaInsets.top)
    }
        ...
        AuthManager.shared.loginUser(username: username, email: email, password: password) { [weak self] (success) in
            DispatchQueue.main.async {
                if success {
                    ...
                    //error occurrred
                    let alert = UIAlertController(title: "Log in Error", message: "We were unable to log u in", preferredStyle: .alert)
